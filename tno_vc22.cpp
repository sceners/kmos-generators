...
unsigned char prog_name[200]="Vocabulizer";
unsigned char date[20]="";
unsigned char about_box_caption[]="The Nameless Ones '2000";
unsigned char first_title[]="Name";
unsigned char second_title[]="Company (Optional)";
unsigned char third_title[]="Key";
...
void GenerateCode(HWND hDlg){
   ...
   char name[100]={0}, company[100]={0}, table2[100]={0}, table[255]="0239488738", alstr[50]={0}, edxstr[100]={0}, code[100]={0}, xiba[100]={0}, final_code[100]={0}, code_string[100]={0};
   int k=0, len_name=0, len_comp=0, length=0, flag=0, flag2=0, length1=0;
   long int al=0, dl=0, edx=0, zzz=0x3DF, tno=0;
   ...
   if (len_name<1)
   {
	...
	strcat(table, alstr);
	strcat(table, table2);
        length1=strlen(table);

        for (k=0;k<16;k++)
        {
                asm{
                        mov eax, [length1]
                        imul edx, [zzz], 8088405h
                        inc edx
                        mov [zzz], edx
                        mul edx
                        mov [tno], edx
