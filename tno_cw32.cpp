...
char prog_name[200]="Cyber-Info WebMail Notify";
char date[11]="";
char about_box_caption[]="About...";
char first_title[]="User Name";
char second_title[]="Password";
...
void GenerateCode(HWND hDlg){
   ...
   int i=0, length=0, eax=1;
   unsigned long total=0, total1=0, ecx=0, esi=0;
   ...
   sprintf(code_string,"%lu%lu",total,total1);
...
BOOL CALLBACK KeygenDialogProcedure(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam){
   ...
   switch(uMsg){
      case WM_SHOWWINDOW:
        ...
        //
        // Check for SoftICE in registry! Coded bY Kmos...
        //
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE,"SOFTWARE\\NuMega\\SoftICE",0,KEY_ALL_ACCESS,&hKey)==ERROR_SUCCESS)
        {
                MessageBox(hDlg,
                "SoftICE has been installed in your system!\r\n\r\n"
                "Uninstall SoftICE and remove all files/registry entries!\r\n",
                "WARNING...",MB_OK);
                RegCloseKey(hKey);
                exit(0);
         }
