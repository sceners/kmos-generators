...
void GenerateCode(HWND hDlg){
   ...
   unsigned char name[100]={0}, aname[100]={0}, xname[100]={0}, code[50]={0}, codez[50]={0}, code_string[100]={0}, code_s[100]={0};
   int i=0, length=0, length1=0, flag=0;
   unsigned long int al=0, edx=0, edi=0;
   ...
   if (length < 1) {
	...
	for (i=0;i<4;i++)
	{
		code_string[8+i]=codez[4+i];
        }

   sprintf(code_s,"KS%s",code_string);
   SetWindowText(hwndCode,code_s);
}
