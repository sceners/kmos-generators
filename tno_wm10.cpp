...
char mycomp[]="KMOSRULEZTNO";
char prog_name[200]="WebMinimizer *ENGLiSH*";
char date[11]="";
char about_box_caption[]="About...";
char first_title[]="Username";
char second_title[]="Key 1";
char third_title[]="Key 2";
...
void GenerateCode(HWND hDlg){
   ...
   SHA_CTX context;
   unsigned char name[100], company[100]={0}, buffer[100], hash[20], *addon="TSM Chall start", *bah="Xu42-w56H6ghS-8Mli", *bah1="TSM Inc copyright 1998", code_string[50]={0}, code[50]={0}, code_x[50]={0}, code_y[50]={0};
   unsigned int i=0, j=0, length=0, len_name=0, len_comp=0;
   ...
   if (len_name < 1)
   {
        SetWindowText(hwndCode,"Enter your Username...");
        return;
   }
	...
	strcat(code_y,bah1);
	strcat(code_y,code_x);
	strcat(code_y,"00");
	strcat(code_y,bah);

	length=strlen(code_y);

	SHA1_Init(&context);
	SHA1_Update(&context, code_y, length);
	SHA1_Final(hash, &context);
