...
void GenerateCode(HWND hDlg){
   ...
   MD5_CTX context;
   unsigned char digest[16]={0}, buffer[50]={0}, name[100]={0}, *addon="CoolLabel", code[256]={0}, code_string[256]={0};
   int i=0, length=0;
   ...
   if (length < 6)
   {
        SetWindowText(hwndCode,"Enter your Name... (min 6)");
	return;
   }

   if (length > 32)
   {
        SetWindowText(hwndCode,"Your Name is too long... (max 32)");
	return;
   }
	...
	for (i=0;i<8;i++)
	{
		code[i]=code_string[i];
	}
	code[8]='-';

	for (i=9;i<17;i++)
	{
		code[i]=code_string[i-1];
	}
