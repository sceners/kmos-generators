...
void GenerateCode(HWND hDlg){
   ...
   char name[100]={0}, code_string[100]={0};
   int i=0, length=0, flag=0, code1=0, code2=0, code3=0;
   ...
   {
        checksum=name[i];
        total=(total + checksum) * 2;
   }

   sprintf(code_string,"%d145h%d%lu%d6543",code1,code2,total,code3);
...
BOOL CALLBACK KeygenDialogProcedure(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam){
   ...
   switch(uMsg){
      ...
      case WM_SHOWWINDOW:
         ...
         MessageBox(hDlg,
            "Hi dude! To register the program, you only need\r\n"
            "the Key Code, but to generate it you need enter\r\n"
            "your name! Ok?\r\n\r\n"
            "                                                   Kmos '2000",
            "iNFORMATiON",MB_OK);
