...
char prog_name[200]="IPMonitor";
char date[11]="";
char about_box_caption[]="About...";
char first_title[]="Name";
char second_title[]="Serial Number";
...
void GenerateCode(HWND hDlg){
   ...
   unsigned char name[100]={0}, code[100]={0}, code_string[100]={0};
   int i=0, length=0;
   unsigned long int checksum=0, ecx=0, total1=0, total2=0;
   ...
   if (length < 3)
   {
        SetWindowText(hwndCode,"Enter your Name... (min 3)");
	return;
   }
	...
	total1=checksum * 0xCCCCCCCC;
	sprintf(code,"%lX",total1);
	strcat(code_string,code);
	strcat(code_string,"-");
        total2=checksum * 0x56805000;
	sprintf(code,"%lX",total2);
	strcat(code_string,code);
...
BOOL CALLBACK KeygenDialogProcedure(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam){
   ...
   switch(uMsg){
      case WM_SHOWWINDOW:
         ...
         MessageBox(hDlg,
            "You must put a * in the start of your name and\r\n"
            "a * in the end of your name, it's a program check!!!\r\n",
            "Keygen information...",MB_OK);
