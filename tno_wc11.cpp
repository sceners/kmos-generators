...
void GenerateCode(HWND hDlg){
   ...
   unsigned char name[100]={0}, code[100]={0}, code_string[100]={0}, table[]="ABCDEF123456789";
   int i=0, length=0, flag=0;
   unsigned long int edx=0, esi=0, ecx=0, edi=0, ebx=0, eax=0, xf40c=0, xf408=0, total_esi=0, x_esi=0, x_eax=0, x_ebx=0;
        ...
        for (i=0;i<length;i++)
	{
		...
		asm{
			...
			mov ebx, edx
                ...
                x_eax=x_ebx * 2 + x_esi;
                x_eax=table[x_eax + x_ebx];
                sprintf(code,"%c",x_eax);
