...
char prog_name[200]="Febo";
char date[11]="";
char about_box_caption[]="The Nameless Ones '2000";
char first_title[]="Serial Number";
...
void GenerateCode(HWND hDlg){
   ...
   unsigned char serial[10]={0}, final_serial[10]={0}, code_string[50]={0};
   int i=0, length=0, flag=0;
   unsigned long int total=0;

   // Get user name from input box
   hwndName=GetDlgItem(hDlg,IDC_NAMEEDIT);
   SendMessage(hwndName,10,sizeof(serial),0);
   ...
   //
   GetWindowText(hwndName,serial,sizeof(serial));
   hwndCode=GetDlgItem(hDlg,IDC_CODEEDIT);

   length=strlen(serial);

   if(length<9) {
      SetWindowText(hwndCode,"Enter your Serial Number. Get it at Febo!...");
      return;
   }

        final_serial[0]=serial[8];
        final_serial[1]=serial[3];
        final_serial[2]=serial[4];
	final_serial[3]=serial[0];
	final_serial[4]=serial[7];
	final_serial[5]=serial[2];
	final_serial[6]=serial[5];
	final_serial[7]=serial[1];
	final_serial[8]=serial[6];

	total=atol(final_serial);
	total+=0x4728324;
