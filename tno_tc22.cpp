...
char prog_name[200]="TrojanCheck";
char date[11]="";
...
void GenerateCode(HWND hDlg){
   ...
   unsigned char name[80]={0}, code[80]={0}, code_string[250]={0}, temp[80]={0};
   int i=0, k=1, length=0, len_temp=0, dupes=0;
   unsigned long ecx=0;
   ...
   if (length > 66)
   {
        SetWindowText(hwndCode,"Your Name is too long... (max 66)");
	return;
   }

	strupr(name);
	strcat(code_string,"TC-");

	for (k=1,i=0;i<length;i++)
	{
		if ((name[i]==0x20) && (name[i+1]>0))
		{
			temp[k]=temp[k] + name[i+1];
			k++;
		}
		else temp[0]=name[0];
	}
