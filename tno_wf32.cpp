...
void GenerateCode(HWND hDlg){
   ...
   char name[100]={0}, code_string[100]={0}, esi, neime[100]={0}, finalneime[100]={0};
   int i=0, length=0, flag=0;
   long int eax=0, ecx=0, esp=0, ebx=0;
   ...
   ltoa (ebx, neime, 10);
   //
   for (i=0;i<strlen(neime)-1;i++)
   	finalneime[i]=neime[i];
 		finalneime[strlen(neime)-1]='\0';
   ebx=atol(finalneime);
   //
   eax=ebx * ebx;
   eax=eax + esp;

   sprintf(code_string,"%lu",eax);
