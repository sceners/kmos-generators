...
char myname[]="Kmos [tNO '99]";
char prog_name[200]="Chat 99";
char date[11]="";
char about_box_caption[]="The Nameless Ones '99";
char first_title[]="Name";
char second_title[]="License Key";
...
void GenerateCode(HWND hDlg){
   ...
   static unsigned char *xiba="chat 9";
   char yeah[50]={0}, kmos[100]={0}, code_string[100]={0};
   unsigned char name[100]={0}, namez[100]={0}, tno[100]={0};
   unsigned char final_code[20]="196927051997", code_final[100]={0};
   int i=0, j=0, length=0, length1=0, xxx=0, fuck1=0, flag=0;
   ...
   strlwr(name); // Convert name to lowercase!
   ...
   length1=strlen(namez);
   fuck1=length1;

   for (i=0;i<length1;i++,fuck1--)
   {
	   yeah[i]=namez[fuck1-1];
	   xxx=yeah[i]+1-1;

	   itoa(xxx, kmos, 10);

	   final_code[fuck1-1]=kmos[1];
   }

   for (i=0;i<4;i++,j++)
   {
	   code_final[i]=final_code[j];
   }
   code_final[4]='-';

   for (j=4,i=0;i<2;i++,j++)
   {
	   code_final[j+1]=final_code[j];
   }
   ...
   for (i=0;i<6;i++,j++)
   {
	   code_final[j+4]=final_code[j];
   }

   sprintf(code_string,"%s",code_final);
...
BOOL CALLBACK AboutDialogProcedure(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam){
   switch(uMsg){
      ...
      case WM_SHOWWINDOW:
         ...
         strcat(about_box_text,"\r\n\r\n");
         strcat(about_box_text,"The Nameless Ones '99\r\n");
