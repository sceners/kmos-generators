...
void GenerateCode(HWND hDlg){
   ...
   unsigned char name[100]={0}, code[100]={0}, code_string[100]={0}, code1[100]={0};
   int i=0, length=0, flag=0;
   unsigned long int eax=0, edx=0, ecx=0, edi=0, ebx=0, ebp=0;
   ...
   if(length<1) {
	...
	strcat(code_string,"SB1-");
	sprintf(code,"%lu%d",edi,ebx);
	strcat(code_string,code);
	strcat(code_string,"-");
	sprintf(code1,"%d-%lu",name[0],ebp);
