...
char prog_name[200]="Q-Recovery";
char date[11]="";
char about_box_caption[]="About...";
char first_title[]="Owner";
char second_title[]="Company";
char third_title[]="Registration Code";
...
void GenerateCode(HWND hDlg){
   ...
   unsigned char name[100]={0}, company[100]={0}, code_string[100]={0};
   int i=0, len_name=0, len_comp=0, flag=0, flag2=0;
   unsigned long int ecx=0, eax=0, len_total, k1=0, k2=0;
   ...
   if (len_name < 3)
   {
        SetWindowText(hwndCode,"Enter your Owner (min 3)...");
        return;
   }
   ...
   if (len_comp < 1)
   {
	...
	len_total=9 + (len_name + len_comp);

	k1=((name[1]&0xFF) + (name[2]&0xFF));

        k2=((name[0]&0xFF) + (company[0]&0xFF));

        k2=k2 * 666;
        lx=ldiv(k2,100);
	k2=lx.rem;

   sprintf(code_string,"TNOKS-%lu%lu%lu",len_total,k1,k2);
