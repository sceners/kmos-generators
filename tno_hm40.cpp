...
void GenerateCode(HWND hDlg){
   ...
   unsigned char name[100]={0}, namez[100]={0}, ten_string[]="HEXMINESVERO", xiba[100]={0}, bah[12]={0}, tno[12]={0}, tnok[50]={0}, tnoy[50]={0}, code_string[100];
   int i=0, length=0, flag=0;
   unsigned long int al=0, eax=0, edi=0, tnoz=0;
   ...
   if(length<1) {
	...
	for (i=0;i<8;i++)
	{
		...
		{
			tno[0]=0x31;
			sprintf(tnoy,"%c",tno[0]);
			strcat(bah,tnoy);
			sprintf(tnok,"%c",xiba[0]);
			strcat(bah,tnok);
		}
		else
		{
